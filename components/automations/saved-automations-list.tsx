...
function describeCron(cron: string): string {
  ...
  let desc = `${hour}:${min.padStart(2, "0")}`
  if (dom !== "*") desc += ` dia ${dom}/mes`
  else if (dow === "1-5") desc += " Seg-Sex"
  else if (dow === "1") desc += " Segundas"
  else if (dow === "*") desc += " Diario"
  else desc += ` dow:${dow}`
  return desc
}
                ...
                <div className="flex flex-1 flex-col gap-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      {format(new Date(auto.created_at), "dd/MM/yy HH:mm", {
                        locale: ptBR,
                      })}
                    </span>
                  </div>
