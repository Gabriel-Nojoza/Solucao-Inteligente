...
import type { Automation, Contact, DatasetColumn, QueryFilter } from "@/lib/types"
...
export function DuplicateReportDialog({
  ...
  useEffect(() => {
    ...
    setName(`${report.name} - copia`)
    setCron(report.cron_expression || defaultCron)
    setEnableSchedule(Boolean(report.cron_expression))
  ...
  function updateFilterValue(id: string, field: string, value: string) {
    setLocalFilters((current) =>
  ...
  async function handleDuplicate() {
    if (!report.dataset_id) {
      toast.error("Este relatorio nao possui dataset vinculado para copiar")
      return
    }

    if (!name.trim()) {
      toast.error("Informe um nome para a copia")
      return
    }
    ...
    try {
      const res = await fetch("/api/automations", {
        ...
        body: JSON.stringify({
          ...
          selected_columns: Array.isArray(report.selected_columns) ? report.selected_columns : [],
          selected_measures: Array.isArray(report.selected_measures) ? report.selected_measures : [],
          filters: localFilters,
          dax_query: report.dax_query || null,
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Erro ao criar copia do relatorio")
      }

      await globalMutate("/api/automations")
      toast.success("Copia do relatorio criada com sucesso!")
      setOpen(false)
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Erro ao criar copia do relatorio"
      )
    } finally {
      setSaving(false)
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[88vh] overflow-hidden sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>Copiar {report.name}</DialogTitle>
          <DialogDescription>
            Crie uma nova copia deste relatorio e ajuste nome, envio, filtros e contatos.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label>Nome da copia</Label>
                <Input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Ex: Relatorio mensal - copia"
                />
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Checkbox
                  id={`duplicate-schedule-${report.id}`}
                ...
                />
                <Label htmlFor={`duplicate-schedule-${report.id}`}>
                  Agendar envio automatico
                </Label>
              </div>
              {enableSchedule ? <CronBuilder value={cron} onChange={setCron} /> : null}
            </div>
            ...
            <div className="space-y-3 rounded-lg border border-border/60 p-3">
              <div>
                <p className="text-sm font-medium">Filtros da copia</p>
                <p className="text-xs text-muted-foreground">
                  Ajuste os filtros antes de criar a nova versao do relatorio.
                </p>
              </div>
                  ...
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          Ajuste os valores que vao para a copia
                        </p>
                      </div>
                    </div>
                          ...
                          >
                            ...
                            {isDateFilter ? (
                                ...
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <div className="space-y-1">
                                    ...
                                    <Input
                                      ...
                                      value={filter.value}
                                      onChange={(e) =>
                                        updateFilterValue(filter.id, "value", e.target.value)
                                      }
                                      className="h-9"
                                    />
                                  </div>
                                  <div className="space-y-1">
                                    ...
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        updateFilterValue(filter.id, "valueTo", e.target.value)
                                      }
                                      className="h-9"
                                    />
                                  </div>
                              ...
                              <FilterValueField
                                ...
                                onUpdateFilter={updateFilterValue}
                              />
                            )}
            ...
            <div className="space-y-2 rounded-lg border border-border/60 p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Escolha os contatos que devem ficar vinculados na copia.
                </p>
              </div>
