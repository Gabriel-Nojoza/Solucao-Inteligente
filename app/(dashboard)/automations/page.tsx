...
export default function AutomationsPage() {
  ...
  const executeQuery = useCallback(async (options?: { silent?: boolean }) => {
  ...
  }, [daxQuery, hasQuery, pbiWorkspaceId, selectedDataset, selectedExecutionDataset])

  const saveExecutionDatasetMapping = useCallback(async (executionDatasetId: string) => {
    if (!selectedDataset || !pbiWorkspaceId) return

    const executionDataset = datasets.find((dataset: { id: string; name: string }) => dataset.id === executionDatasetId)
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || "Erro ao salvar dataset auxiliar")
  ...
  const handleGeneratePdf = useCallback(() => {
    ...
    if (!printWindow) {
      ...
      toast.error("Nao foi possivel abrir a janela do PDF. Verifique se o navegador bloqueou pop-up.")
      return
    }
  ...
  const importCatalogFromScanner = async () => {
    ...
    try {
      ...
      await mutateFixedCatalog()
      toast.success(
        `Catalogo importado: ${data.table_count ?? 0} tabelas, ${data.column_count ?? 0} colunas, ${data.measure_count ?? 0} medidas`
      )
    } catch (error) {
      toast.error(
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-border bg-muted/30 px-3 py-2 sm:px-4">
            ...
            {isLoadingSchema && (
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                {loadingFixedCatalog ? "Carregando catalogo salvo..." : "Carregando metadados..."}
              </div>
            )}

            {selectedDataset && selectedExecutionDataset && selectedExecutionDataset !== selectedDataset && (
