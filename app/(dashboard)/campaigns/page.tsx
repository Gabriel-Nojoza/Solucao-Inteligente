...
import {
  ...
  ImageIcon, X, Clock, Users, MessageSquare, Send, CalendarClock,
  History, CheckCircle2, XCircle, AlertCircle, ChevronLeft, Check, Eye,
} from "lucide-react"
...
type PreviewClient = { name: string | null; phone: string | null; data?: Record<string, unknown> }
...
function resolveFormMessage(template: string, data: Record<string, unknown>): string {
  return template.replace(/\{\{(\w+)\}\}/g, (_, key: string) => formatPreviewValue(data[key]))
}
...
export default function CampaignsPage() {
  ...
  const [previewFormClient, setPreviewFormClient] = useState<PreviewClient | null>(null)
  ...
  if (viewMode === "form") {
    return (
      <div className="flex flex-1 flex-col">
                            ...
                            {loadingPreview ? (
                              ...
                              <>
                                <div className="flex border-b">
                                  <button
                                    type="button"
                                    className="flex-1 border-r bg-emerald-600 py-2 text-xs font-semibold text-white hover:bg-emerald-700 transition-colors"
                                  ...
                                  <button
                                    type="button"
                                    className="flex-1 py-2 text-xs font-semibold text-muted-foreground bg-muted/30 hover:bg-muted/50 transition-colors"
                                    onClick={() => setRemovedIndexes(new Set(previewClients.map((_, i) => i)))}
                                  >
                                    Desmarcar Todos
                                  </button>
                                </div>
                                <div className="max-h-96 overflow-y-auto divide-y divide-border/40">
                                  {previewClients.map((client, i) => {
                                    const selected = !removedIndexes.has(i)
                                    ...
                                    const tags = [...priority, ...rest].slice(0, 4).map(([k, v]) => `${colName(k)}: ${String(v)}`)
                                    return (
                                      <div
                                        key={i}
                                        role="button"
                                        ...
                                        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { const next = new Set(removedIndexes); if (next.has(i)) next.delete(i); else next.add(i); setRemovedIndexes(next) } }}
                                        className={`flex w-full items-center gap-2.5 rounded-lg mx-1 my-0.5 px-2.5 py-2 transition-colors cursor-pointer ${
                                          selected ? "bg-muted/20 hover:bg-muted/40" : "opacity-40 bg-muted/10 hover:bg-muted/20"
                                        }`}
                                      >
                                        <div className={`shrink-0 size-3.5 rounded border flex items-center justify-center transition-colors ${
                                        ...
                                        <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                                          ...
                                          {tags.length > 0 && (
                                            <span className="flex flex-wrap gap-1 mt-0.5">
                                              {tags.map((tag, ti) => (
                                                <span key={ti} className="text-[10px] bg-muted px-1.5 py-0.5 rounded font-medium text-muted-foreground">{tag}</span>
                                              ))}
                                            </span>
                                          )}
                                        </div>
                                        <button
                                          type="button"
                                          onClick={(e) => { e.stopPropagation(); setPreviewFormClient(client) }}
        ...
        {previewFormClient && (
          <Dialog open={!!previewFormClient} onOpenChange={() => setPreviewFormClient(null)}>
            <DialogContent className="max-w-lg">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm font-medium">{previewFormClient.name || "Sem nome"}</p>
                  <p className="text-xs text-muted-foreground">{previewFormClient.phone || "Sem telefone"}</p>
                </div>
                <div className="flex flex-col gap-1">
                  ...
                  <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-5 py-4">
                    <p className="text-sm leading-relaxed whitespace-pre-wrap">
                      {resolveFormMessage(form.message_template, previewFormClient.data ?? {})}
                    </p>
                  </div>
                </div>
                {Object.keys(previewFormClient.data ?? {}).length > 0 && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="rounded-xl border bg-muted/20 px-4 py-2 max-h-72 overflow-y-auto">
                      {Object.entries(previewFormClient.data ?? {}).map(([key, val]) => {
                        ...
                        return (
                          <div key={key} className="flex justify-between gap-6 py-2 border-b border-border/30 last:border-0">
                            ...
                            <span className="text-sm font-semibold text-right">
                              {val != null && String(val).trim() !== "" ? formatPreviewValue(val) : "—"}
                            </span>
                          </div>
                        )
                      })}
