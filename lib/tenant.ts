...
function getCompanyId(user: { app_metadata?: Record<string, unknown>; user_metadata?: Record<string, unknown> }) {
  const appCompanyId = user.app_metadata?.company_id
  const userCompanyId = user.user_metadata?.company_id
  const companyId = typeof appCompanyId === "string" ? appCompanyId : typeof userCompanyId === "string" ? userCompanyId : ""
  return companyId.trim()
}

function isPlatformAdminEmail(email: string) {
  const configured = (process.env.PLATFORM_ADMIN_EMAIL || "admin@seuapp.com").trim().toLowerCase()
